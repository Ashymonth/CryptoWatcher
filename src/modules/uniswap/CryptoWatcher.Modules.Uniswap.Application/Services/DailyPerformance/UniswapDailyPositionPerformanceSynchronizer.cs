...
namespace CryptoWatcher.Modules.Uniswap.Application.Services.DailyPerformance;
    ...
    IDailyPositionPerformanceSynchronizer
{
        ...
        IReadOnlyCollection<Wallet> wallets, DateOnly from, DateOnly to, CancellationToken ct)
    {
        ...
        {
            if (uniswapLiquidityPosition.Snapshots.Count == 0)
            {
                continue;
            }
            ...
            foreach (var currentSnapshot in uniswapLiquidityPosition.Snapshots)
            {
